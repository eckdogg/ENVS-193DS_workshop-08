...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA increases, total plant mass tends to increase \n (positive relationship)")
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "Mean plant mass is higher in well watered than \ndrought stressed treatment") +
  # taking out legend
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Species",
       y = "Total mass (g)",
       title = "Species differ in mean total mass") +
...
```{r model-summary}
summary(model4)
# reference level for water_treatment: Drought stressed (doesn't show up)
# reference level for species_name: Lotus scoparius (doesn't show up)
```
...
**What is the best model?**
 ...
 The best model that explains total plant mass includes water treatment and species (F(7,62)) = 27.08, p < 0.001, $\$alpha$ = 0.05), but not SLA.

**How much variation in the response (total mass, in grams) does this model explain?**

This model explains 73% (adjusted R^2^) of the variation in total mass.
...
model4_predictions <- ggpredict(
  model4,
  terms = c("species_name", "water_treatment") # listing predictors
) |> 
  # rename columns (treating this like a data frame)
